  ...
  return (
    <div className="fade-up grid grid-cols-[clamp(160px,18vw,240px)_1fr_auto] max-[800px]:grid-cols-[100px_1fr] gap-[clamp(20px,2.5vw,40px)] py-[clamp(20px,2.5vw,32px)] border-b border-rule items-center transition-[background-color,padding-left,padding-right] duration-300 hover:bg-bg-2 hover:px-3 max-[800px]:hover:px-0">
      ...
      <div>
        ...
        <div className="text-[clamp(13px,calc(0.2vw+12px),17px)] text-ink-2 mt-1">
          {company}
        </div>
      </div>
      ...
      <div className="border-t border-rule">
        ...
        <article className="fade-up grid grid-cols-[1.05fr_1fr] max-[800px]:grid-cols-1 gap-[clamp(32px,5vw,72px)] py-[clamp(36px,5vw,72px)] border-b border-rule items-center group">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="font-mono text-[13px] text-ink-3">
              ( KreativKat digital marketing )
            </div>
            <p className="text-[clamp(15px,calc(0.4vw+13px),20px)] leading-relaxed text-ink-2 m-0 max-w-[52ch]">
              Led the technical side of a digital marketing agency in cebu —
              scoping client builds, architecting the stack, reviewing code,
              mentoring the team, and unblocking everyone so we could ship
              faster.
            </p>
            <div className="flex flex-wrap gap-1.5">
              {["Team lead", "next.js", "node", "api integration", "SEO"].map(
                (t) => (
                  <span key={t} className="tag">
                    {t}
                  </span>
        ...
        <article className="fade-up grid grid-cols-[1.05fr_1fr] max-[800px]:grid-cols-1 gap-[clamp(32px,5vw,72px)] py-[clamp(36px,5vw,72px)] border-b border-rule items-center group">
          <div className="flex flex-col gap-4 max-[800px]:order-2">
            ...
            <p className="text-[clamp(15px,calc(0.4vw+13px),20px)] leading-relaxed text-ink-2 m-0 max-w-[52ch]">
              Worked as a full stack developer at staff outsourcing solutions
              for a year. built and maintained client websites and applications
              across react, node, and mongodb stacks.
            </p>
            <div className="flex flex-wrap gap-1.5">
              {["react", "node", "express", "mongodb", "azure", "firebase"].map(
                (t) => (
                  <span key={t} className="tag">
                    {t}
                  </span>
